...
.DESCRIPTION
    This script logs explicit credential usage (Event ID 4648) from Security EVTX files in a selected folder 
    using COM-based LogQuery, generating a consolidated CSV report with user-configurable settings via a GUI.
...
function Compile-ExplicitCredentialEvents {
    ...
    Write-Log "Starting to compile explicit credential usage events (Event ID 4648) in folder '$LogFolderPath'"
    ...
    try {
        ...
        $consolidatedFile = Join-Path $OutputFolder "$DomainServerName-EventID4648-ExplicitCredentialsLogon-$timestamp.csv"
        $tempCsvPath = Join-Path $env:TEMP "temp_explicitcred_$timestamp.csv"
...
"@
            ...
            if (Test-Path $tempCsvPath) {
                ...
                Write-Log "Processed $($file.Name) for explicit credential events"
            }
        }
        ...
        Write-Log "Found $eventCount explicit credential events. Report exported to '$consolidatedFile'"
        ...
        Show-MessageBox -Message "Found $eventCount explicit credential events.`nReport exported to:`n$consolidatedFile" -Title "Success"
    } catch {
        Write-Log -Message "Error compiling explicit credential events: $_" -Level Error
        Show-MessageBox -Message "Error compiling explicit credential events: $_" -Title "Error" -Icon Error
...
$form = New-Object System.Windows.Forms.Form -Property @{
    Text          = 'Explicit Credential Auditor (Event ID 4648)'
...
$button.Add_Click({
    ...
    Compile-ExplicitCredentialEvents -LogFolderPath $evtxFolder -OutputFolder $outputFolder -UserAccounts $userAccounts
})
