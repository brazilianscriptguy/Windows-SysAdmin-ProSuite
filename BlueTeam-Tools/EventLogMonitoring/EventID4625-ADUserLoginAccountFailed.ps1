<#
.SYNOPSIS
    PowerShell Script for Compiling Failed Logon Attempts via Event ID 4625 using Log Parser.

.DESCRIPTION
    This script compiles failed logon attempts (Event ID 4625) from Security EVTX files in a selected folder 
    using COM-based LogQuery, outputting a consolidated CSV report with user-configurable settings via a GUI.
...
function Compile-FailedLogonAttempts {
    [CmdletBinding()]
    param (
        [Parameter(Mandatory)]
    ...
    Write-Log "Starting to compile failed logon attempts (Event ID 4625) in folder '$LogFolderPath'"
    ...
    try {
        ...
        $consolidatedFile = Join-Path $OutputFolder "$DomainServerName-EventID4625-ADLogonFailedAttempts-$timestamp.csv"
        $tempCsvPath = Join-Path $env:TEMP "temp_failedlogon_$timestamp.csv"
...
"@
            ...
            if (Test-Path $tempCsvPath) {
                ...
                Write-Log "Processed $($file.Name) for failed logon attempts"
            }
        }
        ...
        Write-Log "Found $eventCount failed logon attempts. Report exported to '$consolidatedFile'"
        $script:statusLabel.Text = "Completed. Found $eventCount failed logons. Report saved to '$consolidatedFile'"

        if ($AutoOpen -and (Test-Path $consolidatedFile)) { Start-Process -FilePath $consolidatedFile }
        Update-ProgressBar -Value 100
        Show-MessageBox -Message "Found $eventCount failed logon attempts.`nReport exported to:`n$consolidatedFile" -Title "Success"
    } catch {
        Write-Log -Message "Error compiling failed logon attempts: $_" -Level Error
        Show-MessageBox -Message "Error compiling failed logon attempts: $_" -Title "Error" -Icon Error
...
$form = New-Object System.Windows.Forms.Form -Property @{
    Text          = 'Failed Logon Auditor (Event ID 4625)'
...
$button.Add_Click({
    ...
    Compile-FailedLogonAttempts -LogFolderPath $evtxFolder -OutputFolder $outputFolder -UserAccounts $userAccounts
})
