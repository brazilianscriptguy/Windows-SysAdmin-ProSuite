...
function Get-ArchiveSafeEvtxFiles {
    ...
    param(
        ...
        [int]$MaximumFiles = 5000
    )

    $safeFiles = New-Object System.Collections.Generic.List[System.IO.FileInfo]
    $activeSkipped = 0
    $lockedSkipped = 0
    $otherSkipped = 0
    ...
    foreach ($file in @($Files)) {
        try {
            if ($null -eq $file -or [string]::IsNullOrWhiteSpace([string]$file.FullName)) {
                $otherSkipped++
                continue
            }
            ...
            if (Test-IsLikelyActiveEvtxFile -File $file) {
                $activeSkipped++
                Write-Log -Level 'WARNING' -Message ("Skipped active/canonical EVTX in archive mode: {0}" -f $file.FullName)
                continue
            }

            if (Test-IsFileLocked -Path $file.FullName) {
                $lockedSkipped++
                Write-Log -Level 'WARNING' -Message ("Skipped locked EVTX in archive mode: {0}" -f $file.FullName)
                continue
            }
            ...
            $otherSkipped++
            Write-Log -Level 'WARNING' -Message ("Skipped EVTX due to validation failure. Error: {0}" -f $_.Exception.Message)
        }
    }

    $selected = @($safeFiles)
    if ($selected.Count -gt $MaximumFiles) {
        Write-Log -Level 'WARNING' -Message ("Archive file selection exceeded limit. Selected={0}; Limit={1}. Processing newest files only." -f $selected.Count, $MaximumFiles)
        $selected = @($selected | Sort-Object LastWriteTime -Descending | Select-Object -First $MaximumFiles)
    }

    Write-Log ("Archive-safe EVTX selection completed. Selected={0}; ActiveSkipped={1}; LockedSkipped={2}; OtherSkipped={3}" -f $selected.Count, $activeSkipped, $lockedSkipped, $otherSkipped)
    return $selected
}
