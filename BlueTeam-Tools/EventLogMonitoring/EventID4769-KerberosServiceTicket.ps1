<#
.SYNOPSIS
    PowerShell Script for detecting Event ID 4769 (Kerberos service ticket requested).

.DESCRIPTION
    This WS2019-compatible revision analyzes Event ID 4769 from the live Security channel
...
.VERSION
    RevA - WS2019-compatible migration
#>
...
function New-HeaderOnlyCsv {
    ...
    $header = 'EventTime,EventID,TargetUserName,TargetDomainName,ServiceName,TicketOptions,TicketEncryptionType,Status,IpAddress,IpPort,ComputerName,SourceFile'
...
SELECT
    ...
    EXTRACT_TOKEN(Strings, 2, '|') AS ServiceName,
    EXTRACT_TOKEN(Strings, 3, '|') AS TicketOptions,
    EXTRACT_TOKEN(Strings, 4, '|') AS TicketEncryptionType,
    EXTRACT_TOKEN(Strings, 5, '|') AS Status,
    EXTRACT_TOKEN(Strings, 6, '|') AS IpAddress,
    EXTRACT_TOKEN(Strings, 7, '|') AS IpPort,
    ComputerName,
...
function Resolve-SecurityChannel {
    ...
    $tempCsv = Join-Path $env:TEMP ('Security-Probe-4769-{0}.csv' -f (Get-Date -Format 'yyyyMMdd_HHmmss_fff'))
...
function Process-Event4769 {
    ...
    $finalCsv = Join-Path $resolvedOutput ('{0}-EventID4769-KerberosServiceTicket-{1}.csv' -f $computerName, $timestamp)
    $tempCsvFiles = New-Object System.Collections.ArrayList

    Write-Log "Starting Event ID 4769 processing. UseLiveLog=$UseLiveLog; Folder='$EvtxFolder'; IncludeSubfolders=$IncludeSubfolders; OutputFolder='$resolvedOutput'"
    ...
    try {
        ...
        foreach ($source in @($sourceFiles)) {
            ...
            $tempCsv = Join-Path $env:TEMP ('Event4769-{0}-{1}.csv' -f $index, (Get-Date -Format 'yyyyMMdd_HHmmss_fff'))
            Register-TempArtifact -Path $tempCsv
        ...
        Write-Log "Found $rowCount Event ID 4769 records. Report exported to '$finalCsv'"

        if ($AutoOpen -and (Test-Path -LiteralPath $finalCsv -PathType Leaf)) {
            Start-Process -FilePath $finalCsv
        }

        Show-Info -Message ("Found {0} Event ID 4769 records.`r`nReport exported to:`r`n{1}" -f $rowCount, $finalCsv) -Title 'Success'
    }
    catch {
        Write-Log -Level 'ERROR' -Message "Error processing Event ID 4769. $($_.Exception.Message)"
        Update-ProgressSafe -Value 0 -StatusText 'Error occurred. Check log for details.'
        Show-ErrorBox -Message ("Error processing Event ID 4769.`r`n{0}" -f $_.Exception.Message)
    }
    finally {
...
$form.Text = 'Kerberos Service Ticket Detector (Event ID 4769)'
...
$buttonStart.Add_Click({
    ...
    Process-Event4769 -UseLiveLog:$checkUseLive.Checked -EvtxFolder $textEvtx.Text -IncludeSubfolders:$checkIncludeSubfolders.Checked -OutputFolder $textOutput.Text -UserAccounts $users
})
