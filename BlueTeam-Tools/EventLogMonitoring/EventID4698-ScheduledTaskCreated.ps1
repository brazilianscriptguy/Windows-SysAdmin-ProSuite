...
function Resolve-SecurityChannel {
    ...
    $tempCsv = Join-Path $env:TEMP ('Security-Probe-4698-{0}.csv' -f (Get-Date -Format 'yyyyMMdd_HHmmss_fff'))
...
function Process-Event4698 {
    ...
    Write-Log "Starting Event ID 4698 processing. UseLiveLog=$UseLiveLog; Folder='$EvtxFolder'; IncludeSubfolders=$IncludeSubfolders; OutputFolder='$resolvedOutput'"
...
$buttonStart.Add_Click({
    ...
    Process-Event4698 -UseLiveLog:$checkUseLive.Checked -EvtxFolder $textEvtx.Text -IncludeSubfolders:$checkIncludeSubfolders.Checked -OutputFolder $textOutput.Text -UserAccounts $users
})
