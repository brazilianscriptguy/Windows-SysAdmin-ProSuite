...
.DESCRIPTION
    This WS2019-compatible revision analyzes Event ID 4768 from the live Security channel
...
function New-HeaderOnlyCsv {
    ...
    $header = 'EventTime,EventID,TargetUserName,TargetDomainName,ServiceName,TicketOptions,Status,PreAuthType,IpAddress,IpPort,ComputerName,SourceFile'
...
SELECT
    ...
    EXTRACT_TOKEN(Strings, 4, '|') AS Status,
    EXTRACT_TOKEN(Strings, 5, '|') AS PreAuthType,
...
function Resolve-SecurityChannel {
    ...
    $tempCsv = Join-Path $env:TEMP ('Security-Probe-4768-{0}.csv' -f (Get-Date -Format 'yyyyMMdd_HHmmss_fff'))
...
function Process-Event4768 {
    ...
    $finalCsv = Join-Path $resolvedOutput ('{0}-EventID4768-KerberosTGTRequest-{1}.csv' -f $computerName, $timestamp)
    $tempCsvFiles = New-Object System.Collections.ArrayList

    Write-Log "Starting Event ID 4768 processing. UseLiveLog=$UseLiveLog; Folder='$EvtxFolder'; IncludeSubfolders=$IncludeSubfolders; OutputFolder='$resolvedOutput'"
    ...
    try {
        ...
        Write-Log "Found $rowCount Event ID 4768 records. Report exported to '$finalCsv'"

        if ($AutoOpen -and (Test-Path -LiteralPath $finalCsv -PathType Leaf)) {
            Start-Process -FilePath $finalCsv
        }

        Show-Info -Message ("Found {0} Event ID 4768 records.`r`nReport exported to:`r`n{1}" -f $rowCount, $finalCsv) -Title 'Success'
    }
    catch {
        Write-Log -Level 'ERROR' -Message "Error processing Event ID 4768. $($_.Exception.Message)"
        Update-ProgressSafe -Value 0 -StatusText 'Error occurred. Check log for details.'
        Show-ErrorBox -Message ("Error processing Event ID 4768.`r`n{0}" -f $_.Exception.Message)
    }
    finally {
...
$form.Text = 'Kerberos TGT Request Detector (Event ID 4768)'
...
$buttonStart.Add_Click({
    ...
    Process-Event4768 -UseLiveLog:$checkUseLive.Checked -EvtxFolder $textEvtx.Text -IncludeSubfolders:$checkIncludeSubfolders.Checked -OutputFolder $textOutput.Text -UserAccounts $users
})
