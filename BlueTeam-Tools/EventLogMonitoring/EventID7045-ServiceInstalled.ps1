<#
.SYNOPSIS
    PowerShell Script for detecting Event ID 7045 (A service was installed in the system).

.DESCRIPTION
    This WS2019-compatible revision analyzes Event ID 7045 from the live System channel
...
function New-HeaderOnlyCsv {
    ...
    $header = 'EventTime,EventID,ServiceName,ImagePath,ServiceType,StartType,AccountName,ComputerName,SourceFile'
...
function Build-ServiceFilterClause {
    param([string[]]$ServiceNames)

    $normalized = @($ServiceNames | ForEach-Object { "$_".Trim() } | Where-Object { -not [string]::IsNullOrWhiteSpace($_) })
    if (@($normalized).Count -eq 0) {
        return ''
    }

    $escaped = @($normalized | ForEach-Object { "'" + ($_.Replace("'", "''")) + "'" })
    return ("AND EXTRACT_TOKEN(Strings, 0, '|') IN ({0})" -f ($escaped -join '; '))
}
...
function Build-QueryForFile {
    param(
        ...
        [Parameter()][string[]]$ServiceNames
    )

    $escapedEvtx = $EvtxPath.Replace("'", "''")
    $escapedCsv  = $CsvPath.Replace("'", "''")
    $serviceClause  = Build-ServiceFilterClause -ServiceNames $ServiceNames
...
function Invoke-EventQueryToCsv {
    ...
    $query = Build-QueryForFile -EvtxPath $EvtxPath -CsvPath $CsvPath -ServiceNames $ServiceNames
...
function Resolve-SystemChannel {
    ...
    Update-ProgressSafe -Value 10 -StatusText 'Resolving System channel via snapshot export...'
    $snapshot = Export-LiveChannelSnapshot -ChannelName 'System'

    $tempCsv = Join-Path $env:TEMP ('System-Probe-7045-{0}.csv' -f (Get-Date -Format 'yyyyMMdd_HHmmss_fff'))
    Register-TempArtifact -Path $tempCsv
    Invoke-EventQueryToCsv -EvtxPath $snapshot -CsvPath $tempCsv -ServiceNames @()

    if (Test-Path -LiteralPath $tempCsv -PathType Leaf) {
        Write-Log 'Live System channel probe completed successfully.'
        Update-ProgressSafe -Value 0 -StatusText 'Ready.'
        return $snapshot
    }

    throw 'Live System channel probe did not produce a CSV file.'
}

function Process-Event7045 {
    [CmdletBinding()]
    param(
    ...
    $finalCsv = Join-Path $resolvedOutput ('{0}-EventID7045-ServiceInstalled-{1}.csv' -f $computerName, $timestamp)
    $tempCsvFiles = New-Object System.Collections.ArrayList

    Write-Log "Starting Event ID 7045 processing. UseLiveLog=$UseLiveLog; Folder='$EvtxFolder'; IncludeSubfolders=$IncludeSubfolders; OutputFolder='$resolvedOutput'"

    try {
        Update-ProgressSafe -Value 5 -StatusText 'Preparing...'

        if ($UseLiveLog) {
            Update-ProgressSafe -Value 15 -StatusText 'Exporting System snapshot...'
            $snapshot = Export-LiveChannelSnapshot -ChannelName 'System'
        ...
        foreach ($source in @($sourceFiles)) {
            ...
            $tempCsv = Join-Path $env:TEMP ('Event7045-{0}-{1}.csv' -f $index, (Get-Date -Format 'yyyyMMdd_HHmmss_fff'))
            Register-TempArtifact -Path $tempCsv
            [void]$tempCsvFiles.Add($tempCsv)

            Invoke-EventQueryToCsv -EvtxPath $source.FullName -CsvPath $tempCsv -ServiceNames $ServiceNames
        ...
        Write-Log "Found $rowCount Event ID 7045 records. Report exported to '$finalCsv'"

        if ($AutoOpen -and (Test-Path -LiteralPath $finalCsv -PathType Leaf)) {
            Start-Process -FilePath $finalCsv
        }

        Show-Info -Message ("Found {0} Event ID 7045 records.`r`nReport exported to:`r`n{1}" -f $rowCount, $finalCsv) -Title 'Success'
    }
    catch {
        Write-Log -Level 'ERROR' -Message "Error processing Event ID 7045. $($_.Exception.Message)"
        Update-ProgressSafe -Value 0 -StatusText 'Error occurred. Check log for details.'
        Show-ErrorBox -Message ("Error processing Event ID 7045.`r`n{0}" -f $_.Exception.Message)
    }
    finally {
...
$form.Text = 'Service Installed Detector (Event ID 7045)'
...
$checkUseLive.Text = 'Use live System channel'
...
$labelServices.Location = New-Object System.Drawing.Point($left, ($currentY + 3))
$labelServices.Size = New-Object System.Drawing.Size($labelWidth, 20)
$labelServices.Text = 'Service filter:'
$form.Controls.Add($labelServices)

$textServices = New-Object System.Windows.Forms.TextBox
$textServices.Location = New-Object System.Drawing.Point(($left + $labelWidth), $currentY)
$textServices.Size = New-Object System.Drawing.Size($textWidth, 24)
$textServices.Text = ''
$form.Controls.Add($textServices)

$currentY = $currentY + 28

$labelServicesHint = New-Object System.Windows.Forms.Label
$labelServicesHint.Location = New-Object System.Drawing.Point(($left + $labelWidth), ($currentY + 2))
$labelServicesHint.Size = New-Object System.Drawing.Size(430, 18)
$labelServicesHint.Text = 'Separate multiple service names with comma, semicolon, or line break.'
$form.Controls.Add($labelServicesHint)
...
$buttonResolve.Add_Click({
    try {
        Resolve-SystemChannel | Out-Null
        Show-Info -Message 'Live System channel probe completed successfully.' -Title 'Resolve Channel'
    }
    catch {
...
$buttonStart.Add_Click({
    $services = Parse-DelimitedItems -Text $textServices.Text
    Process-Event7045 -UseLiveLog:$checkUseLive.Checked -EvtxFolder $textEvtx.Text -IncludeSubfolders:$checkIncludeSubfolders.Checked -OutputFolder $textOutput.Text -ServiceNames $services
})
