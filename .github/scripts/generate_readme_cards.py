...
OUT_DIR = os.environ.get("README_CARDS_OUT_DIR", "assets/readme-cards")
...
REQUEST_TIMEOUT_SECONDS = int(os.environ.get("GITHUB_API_TIMEOUT_SECONDS", "30"))
...
def gh_graphql(token: str, query: str, variables: dict) -> dict:
    ...
    req.add_header("User-Agent", "Windows-SysAdmin-ProSuite-readme-cards")
...
def main() -> None:
    ...
    if not token:
        die("GITHUB_TOKEN is missing or empty.")
    if not user:
        die("GITHUB_USERNAME is missing or empty.")
    ...
    if gh_repo and (not owner or not repo_name):
        die("GITHUB_REPOSITORY must use the owner/repository format.")
    ...
    out_payload = {
        ...
        "user": u["login"],
        "repository": gh_repo or None,
        "method": "github-linguist-bytes",
        "suiteModules": 10,
        "managedDistributions": 12,
    ...
    footer = "Generated by GitHub Actions | GitHub API"
    ...
    streak_svg = svg_card("Contribution Streak", lines_streak, footer="Computed from GitHub contribution calendar | UTC | 1 year")
